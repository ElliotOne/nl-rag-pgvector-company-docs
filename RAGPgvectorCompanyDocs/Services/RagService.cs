...
public class RagService(TextRepository retriever, Uri ollamaUrl, string modelId = "mistral")
{
    ...
    {
        // Retrieve multiple relevant texts with title, content, and category
        var contexts = await _textRepository.RetrieveRelevantText(query, categoryFilter);

        // Combine title + category + content for context
        string combinedContext = string.Join("\n\n---\n\n", contexts.Select(c =>
            $"Title: {c.Title}\nCategory: {c.Category}\n{c.Content}"
        ));

        // If no relevant context is found, return a strict message
        if (contexts.Count == 1 && contexts[0].Content == "No relevant context found.")
        {
            return new
            {
